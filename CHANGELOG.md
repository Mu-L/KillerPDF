...
- View mode is a rail button: click for a flyout (each mode with its F-key beside it), roll the wheel over it to step through the views, or press F9 to jog from the keyboard - F5-F8 still jump straight to one, and Ctrl+, is retired. All the new shortcuts are on the F1 overlay, in all ten languages.
...
- Switching tabs (or closing one) in the pane that did not have focus could silently repaint the OTHER pane with the wrong document - the tab strip you clicked in still showed the tab you switched to, but its content was replaced by whatever the focused pane was showing. The document, annotations, zoom and the rest of that live state are shared window fields describing whichever pane is focused, not one set per pane, and switching a tab in an unfocused pane wrote into them without first claiming that pane as the focused one. It does now.
...
- Closing the split pane on a maximized window, or a window manually sized to fill the available screen space, shrank the window (or froze one pane at half width until an abrupt snap) instead of leaving pane A filling the space smoothly - because F10 could not have grown the window to make room in the first place, so shrinking it back on close undid space that was never added. Closing now only shrinks the window when opening the split actually grew it; otherwise pane A expands to fill what pane B gave up and the window stays exactly where you left it.
...
- The split's pane size did not survive a restart - reopening after closing while split always came back with the second pane squeezed down to its minimum width, whatever it had been resized to before closing. The restore ran before the window had finished laying out at its restored size, so the divider position it computed from was thrown out; it now waits for that layout pass to finish before dividing the panes.
