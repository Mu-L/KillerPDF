...
- Added the engine's typed object model and recursive parser for scalar values, nested arrays and dictionaries, indirect references, complete `obj`/`endobj` records, and binary-safe stream payloads with direct or resolvable indirect lengths. Source-aware errors enforce nesting, numeric, truncation, and stream-boundary rules.
- Added bounded Flate/zlib stream decoding with 8-bit TIFF and PNG predictor reversal, providing the decoding path required by compressed cross-reference and object streams without allowing unbounded decompression.
- Added final `startxref` discovery plus classic, hybrid, and PDF 1.5–2.0 cross-reference stream parsing. Incremental `/Prev` revisions merge newest-first, inherited trailer values remain available, malformed offsets and cycles are rejected, and future xref entry types safely resolve as null references.
- Added a lazy document loader that resolves current indirect objects through the merged cross-reference table, including indirect stream lengths and PDF 1.5–2.0 `/ObjStm` compressed objects. It validates generations, object headers, object-stream indexes and boundaries, caches decoded object streams, and rejects resolution cycles instead of recursing indefinitely.
- Added bounded, non-throwing structural inspection reports for repair decisions. Diagnostics now distinguish invalid headers, final `startxref` declarations, cross-reference chains, individual indirect objects, and missing or invalid catalog roots, while retaining byte offsets and object numbers for actionable repair messages.
- Added the first preservation-writer layer: deterministic serialization for every current object-model type, with invariant exponent-free numbers, decoded-byte dictionary ordering, canonical name and string escaping, stable LF output, exact binary stream lengths, and a matching nesting safety limit. Equivalent object graphs now produce identical bytes across operating systems.
- Added deterministic full-file rewriting from the merged document view. The writer expands current objects into ordinary indirect objects, removes obsolete xref and object-stream containers, emits a fresh PDF header, classic cross-reference table and sanitized trailer, and produces byte-identical output when its result is reopened and rewritten. Encrypted rewrites fail explicitly until the encryption milestone rather than producing damaged files.
- Added explicit rewrite policy for preserving or upgrading the PDF header version, retaining or removing document information, and independently retaining document identifiers. A reusable round-trip validator and corpus runner now inspect, rewrite, reopen, hash, and rewrite again without modifying the source files.
- Accepted the complete PDF 2.x header declaration range through `%PDF-2.9`, and corrected classic xref `/Size` handling for free boundary entries. Both behaviors were identified against PDF/A-4 conformance fixtures, which also caught and prevented a first-pass/second-pass trailer-size instability.
- Added from-scratch PDF 2.0 catalog and page-tree authoring with deterministic object allocation, arbitrary finite page sizes, blank pages, and page content streams. Typed content construction covers graphics-state save/restore, transforms, paths, Bézier curves, rectangles, line width, grayscale/RGB color, and fill/stroke operations.
- Added validated text objects, positioning, escaped Latin-1 text, and automatic page resource dictionaries for all 14 built-in Type 1 fonts as an authoring baseline.
- Added a bounded TrueType/OpenType reader for font names, metrics, embedding permissions, widths, and Unicode cmap formats 4 and 12. Unicode authoring now embeds permitted TrueType font programs as Type 0/CIDFont resources, emits explicit glyph widths and UTF-16 `ToUnicode` maps (including supplementary-plane characters), and rejects missing or ambiguous glyph mappings instead of silently corrupting text. Deterministic subsetting retains original glyph IDs, recursively includes composite-glyph dependencies, writes valid sfnt checksums, and falls back to full embedding where a font forbids or cannot use glyf/loca subsetting. CFF fonts and complex-script shaping remain explicit later milestones.
- Added image XObject authoring with bounded JPEG header inspection and lossless DCT passthrough for grayscale, RGB, and CMYK images, plus deterministic Flate compression for raw RGB and RGBA pixels. RGBA alpha is preserved as a grayscale soft-mask XObject. Typed placement creates and reuses page resources, supports scaling and mirroring, and keeps binary image data out of the content stream.
- Added Unicode document metadata authoring across both the information dictionary and a catalog XMP metadata stream, including title, author, subject, keywords, creator, producer, language, and timezone-preserving dates. Every newly authored document now receives a stable content-derived 128-bit trailer identifier, so repeated deterministic builds match without omitting PDF identity data.
- Added link annotation authoring for safe external HTTP, HTTPS, and email actions and direct fit-to-page destinations inside the document. Link rectangles are validated, stored as indirect annotations with invisible borders, and unsafe executable or local-file URI schemes are rejected at the API boundary.
- Added Unicode document-outline authoring with fit-to-page bookmark destinations, linked previous/next outline items, accurate counts, and automatic catalog outline mode. Invalid titles and page targets are rejected before serialization.
- Added embedded-file authoring with Unicode file specifications, MIME types, descriptions, sizes, modification dates, PDF 2.0 associated-file relationships, catalog `/AF` registration, and a sorted embedded-files name tree. Payload bytes remain exact, while path traversal and duplicate attachment names are rejected before writing.
- Added AcroForm authoring for text fields, comb fields, checkboxes, radio-button groups spanning multiple pages, and editable or fixed combo boxes. Fields receive stable names and values, page-linked widget annotations, explicit normal appearances, shared default resources, correct behavior flags, and matching on/off appearance states; duplicate names and contradictory field options fail before serialization.
- Added embedded TrueType fonts to text-field and combo-box values and appearances, including Unicode glyph collection across every option, shared AcroForm default resources, deterministic subsetting, Type 0 font references, and two-byte glyph output. A real Arial form containing Unicode text, checkboxes, radio buttons, and a combo box now passes both veraPDF's PDF/A-4 profile and qpdf structural validation.
- Added bounded ICC profile loading for Gray, RGB, and CMYK output intents, including header signature, declared-size, and component validation. A dedicated PDF/A-4 authoring mode now requires metadata and an ICC output intent, writes the PDF/A part/revision identification schema into XMP, omits the forbidden general information dictionary, and produces a from-scratch PDF 2.0 file that passes the veraPDF PDF/A-4 profile and qpdf structural checks.
- Added PDF 2.0 text-note, highlight, underline, strikeout, and squiggly annotation authoring with Unicode contents, stable identities, page links, print flags, colors, opacity, quad points, and explicit appearances. The complete annotation sample passes both veraPDF's PDF/A-4 profile and qpdf structural validation.
- Added embedded-font free-text boxes with multiline wrapping, plus line, rectangle, ellipse, and multi-stroke ink annotation authoring. Each annotation carries editable PDF 2.0 geometry and a deterministic explicit appearance with configurable borders, fills, colors, widths, and opacity; a combined visual sample passes PDF/A-4 conformance and structural validation.
- Added deterministic incremental-update writing for existing PDFs. New and replacement objects append without changing any source byte, reserved references support mutually linked object graphs, nonzero generations and inherited trailer values survive, the `/Prev` revision chain remains valid across classic, hybrid, xref-stream, and compressed-object sources, and the permanent document identifier is retained while the revision identifier advances. Incrementally updated real-world and PDF/A-4 files pass qpdf and veraPDF.
- Added byte-preserving annotation editing for existing PDFs. The editor walks nested page trees, targets individual pages, preserves direct or indirect existing annotation arrays, and incrementally appends text notes plus highlight, underline, strikeout, and squiggly markup with Unicode contents and explicit appearances. A five-annotation update leaves the complete source untouched, renders correctly beside its original annotations, and remains valid PDF/A-4.
