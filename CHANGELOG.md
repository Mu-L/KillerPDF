...
### Changed
- Theme and language moved out of the Settings panel onto the sidebar rail, where the rest of the Killer Tools apps keep them: three new buttons at the bottom of the rail - a ? that opens the keyboard shortcuts overlay (same as F1), a language button, and a theme button - in the same order as KillerNotes, KillerShell and Killendar, below the night-mode moon. Each opens a themed flyout card against the corner of the document pane (never covering the rail, the status bar, or spilling off the window), carrying the exact same pickers as before: every language by its native name, and the six themes with their per-family accent dots. The Settings panel keeps its View, Toolbar, Sidebar and Links sections.
- Right-clicking the title bar (or pressing Alt+Space) now shows a themed system menu that matches the app, instead of the stock white Windows one that ignored every theme. Same items, same behavior - Move and Size still hand off to Windows' own drag loops, Restore/Maximize grey out exactly as before - with the caption glyphs in the left gutter and Close in red. In all ten languages.
- The content pane's border is a shade lighter in every theme, so the edge between the pane and the chrome around it reads more clearly. This brings KillerPDF onto the same value the rest of the Killer Tools apps use.
...
- Internal: the codebase has been reorganized into the Killer Tools family layout (the KillerUI shape Killendar established). All document logic now lives in dedicated service classes - the pre-save scrubs, import and repair, the PDFium interop (one class, one lock), page rasterization, OCR language management, and annotation/stamp burning - with the About, CLI, OCR, and search features behind their own controllers and interfaces, and the window code proper under Shell/. Every moved function is verbatim and no behavior change is intended; for anyone building from source, the repo root now holds only the entry files and the theme resource keys use the family names.
