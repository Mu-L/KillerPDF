...
- Added complete tagged-PDF transfer into empty destinations, preserving the structure tree, parent mappings, document language, accessibility metadata, and viewer preferences. Partial or combined tagged imports and page-set changes that would leave stale structure data now fail explicitly, while complete tagged page sets can still be reordered safely.
