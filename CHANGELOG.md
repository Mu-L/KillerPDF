...
- Name-tree and legacy catalog named-destination merges require non-empty destination arrays or destination dictionaries, remove stale source values before collision handling, reject invalid destination values, and omit stale-only containers.
...
- Sole complete-document transplants require trailer `/Info` to resolve to a dictionary before preserving it.
...
- Complete outline imports recursively validate child lists, item reuse, titles, destinations, actions, associated structure elements, counts, flags, and colors before importing bookmark graphs.
