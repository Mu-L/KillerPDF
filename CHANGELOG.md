...
- Annotations that a PDF already contains - sticky notes, highlights, stamps, ink added in another app - are now displayed (#141, thanks zenfas). They were in the file the whole time and simply never drawn, which is why Firefox and SumatraPDF showed markup that KillerPDF did not. This went further than the screen: flatten, print and image export all rebuild pages from a render, so flattening an annotated PDF silently discarded its annotations and printing left them off the paper. Everything that turns a page into pixels now includes them - viewer, thumbnails, print, flatten, export, page transform - so what you see, print and save agree. OCR is deliberately unchanged: it reads a page's own text, and a reviewer's note is not page content.
