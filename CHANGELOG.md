...
### Changed
- The bookmark sidebar now reads hierarchy, Unicode titles, stable identities, styles, colors, open state, explicit destinations, and named destinations through The KillerPDF.Engine. Tree rows and multi-selection retain engine data instead of live PdfSharpCore outline objects, while unfinished editing commands resolve legacy objects only for the duration of each edit.
